...
ALTER PROCEDURE TrataErroZeroTry2 @CPF VARCHAR(12), @ANO INT, @DENOMINADOR INT, @MENSAGEM VARCHAR(MAX) OUTPUT
AS
BEGIN
  BEGIN TRY
   SELECT SUM(A.QUANTIDADE * A.PRECO) AS FATURAMENTO, SUM(A.QUANTIDADE * A.PRECO) / @DENOMINADOR AS COMISSAO
...
DECLARE @MENSAGEM VARCHAR(MAX)
EXEC InclusaoVendedor03 '00242', 'Pericles Alves', 0.11, '20160821', 0, 'Santo Amaro', @MENSAGEM = @MENSAGEM OUTPUT
IF @MENSAGEM <> ''
    PRINT @MENSAGEM
