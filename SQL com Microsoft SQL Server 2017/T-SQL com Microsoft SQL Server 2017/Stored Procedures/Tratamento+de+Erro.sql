...
SELECT SUM(A.QUANTIDADE * A.PRECO) AS FATURAMENTO, SUM(A.QUANTIDADE * A.PRECO)/10 AS COMISSAO
FROM ITENS_VENDIDOS A INNER JOIN NOTAS B
ON A.NUMERO = B.NUMERO WHERE B.CPF = '1471156710' AND YEAR(B.DATA) = 2018


DECLARE @DENOMINADOR INT
SET @DENOMINADOR = 0
SELECT SUM(A.QUANTIDADE * A.PRECO) AS FATURAMENTO, SUM(A.QUANTIDADE * A.PRECO) / @DENOMINADOR AS COMISSAO
FROM ITENS_VENDIDOS A INNER JOIN NOTAS B
ON A.NUMERO = B.NUMERO WHERE B.CPF = '1471156710' AND YEAR(B.DATA) = 2018

CREATE PROCEDURE TrataErroZero @CPF VARCHAR(12), @ANO INT, @DENOMINADOR INT, @NUMERRO INT OUTPUT, @NUMLINHA INT OUTPUT
AS
BEGIN
...
DECLARE @NUMERRO INT
DECLARE @NUMLINHA INT
EXEC InclusaoVendedor '00238', 'Pericles Alves', 0.11, '20160821', 0, 'Santo Amaro', @NUMERRO = 
