...
IF OBJECT_ID('TAB_NUMEROS', 'U') IS NOT NULL
DROP TABLE TAB_NUMEROS
CREATE TABLE TAB_NUMEROS (NUMERO INT, STATUS VARCHAR(200))
...
WHILE @LIMITE_MINIMO <= @LIMITE_MAXIMO
BEGIN
   SELECT @CONTADOR_NOTAS = COUNT(*) FROM NOTAS WHERE NUMERO = @LIMITE_MINIMO
   IF @CONTADOR_NOTAS > 0 
       INSERT INTO TAB_NUMEROS (NUMERO, STATUS) VALUES (@LIMITE_MINIMO, 'É nota fiscal')
   ELSE
       INSERT INTO TAB_NUMEROS (NUMERO, STATUS) VALUES (@LIMITE_MINIMO, 'Não é nota fiscal')
   SET @LIMITE_MINIMO = @LIMITE_MINIMO + 1   
END
...
BEGIN
    SELECT @NUMNOTAS = COUNT(*) FROM NOTAS WHERE DATA = @DATAINICIAL
